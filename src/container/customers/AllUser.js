...
const ListUser = () => {
    ...
    },
    {
      ...
      render: (text, record) => (
        <LazyLoadImage
          effect="blur"
          width="40"
          src={renderImage(record.id, record.image, 'user', '128x128', true)}
  ...
  const onDateRangeChange = dateRange => {
    if (dateRange) {
      setFilterParams(s => ({
        ...s,
        customerEntryStartDate: dateRange[0]._d ?? '',
        customerEntryEndDate: dateRange[1]._d ?? '',
      }));
              ...
              {staffs.loading ? (
                ...
                <>
                  <Row gutter={25}>
                    ...
                    <Col span={6}>
                      <Button size="large" type="primary" disabled={searchDisable} onClick={searchCustomerAdmin}>
                        Search
                      </Button>
                    </Col>
                  </Row>
                      ...
                      <Row gutter={16}>
                        <Col span={8}>
                          ...
                          <Select
                            ...
                            mode="single"
                            value={
                              filterParams?.customerStatus === 'true'
                                ? true
                                : filterParams?.customerStatus === 'false'
                                ? false
                                : ''
                            }
                            onDeselect={val =>
                              setFilterParams(prev => ({
                                ...prev,
                                customerStatus: prev.customerStatus.filter(item => item !== val),
                              }))
                            }
                            onSelect={val => {
                        ...
                        <Col span={8}>
                          ...
                          <Select
                            ...
                            mode="single"
                            value={
                              filterParams?.emailVerified === 'true'
                                ? true
                                : filterParams?.emailVerified === 'false'
                                ? false
                                : ''
                            }
                            onDeselect={val =>
                              setFilterParams(prev => ({
                                ...prev,
                                emailVerified: prev.emailVerified.filter(item => item !== val),
                              }))
                            }
                            onSelect={val => {
