...
const ListAttributes = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: (a, b) =>
        a.attribute_group.attr_group_name.toUpperCase() > b.attribute_group.attr_group_name.toUpperCase() ? 1 : -1,
  ...
  const onChangeSearch = e => {
    ...
    setFilteredAttributes(
      attributes.data.filter(attr =>
        (attr?.attribute_name + attr?.attribute_group?.attr_group_name).toLowerCase().includes(value.toLowerCase()),
      ),
    );
  };
  ...
  const handleStatusChange = (record, checked) => {
    ...
    apolloClient
      ...
      .then(res => {
        ...
        if (!data?.status) return InternalErrorMessage();
        message.success(`${record.attribute_name} attribute status updated.`);
      });
  };
