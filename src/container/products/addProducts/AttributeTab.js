...
const AttributeTab = ({ attributesTableData, setAttributesTableData }) => {
    ...
    },
    {
      ...
      render: (val, record, index) => (
        <Select
          style={{ width: '100%' }}
          defaultValue={val}
          placeholder={attributeGroups.isLoading ? 'Loading...' : 'Select Attribute...'}
          options={attributeGroups?.data
            .filter(item => item.id === record.attr_group_id)[0]
            ?.attributes?.map(item => ({
          ...
          return (
            ...
            >
              ...
              <p style={{ fontSize: 12, marginTop: 10 }}>
                {record.attribute_value.name ? `${record.attribute_value.name}` : `${record.attribute_value}`}
              </p>
            </Upload>
          );
        }
  ...
  return (
    <>
      <Table columns={column} dataSource={attributesTableData} pagination={false} rowKey="attribute_value" />
