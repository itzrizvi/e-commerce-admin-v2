...
const ListCoupon = () => {
  ...
  const dummyData = [...Array(10).keys()].map(i => ({
    ...
    status: true,
    d:
      'Lorem ipsum, dolor sit amet consectetur adipisicing elit. Atque, architecto quod quidem fuga assumenda temporibus quam quae sapiente cupiditate voluptatum.',
    so: i + 1,
  }));
  ...
  const onChangeSearch = e => {
    ...
    setFilteredCoupons(
      coupons.data.filter(coupon =>
        (coupon?.coupon_name + coupon.coupon_description + coupon.coupon_code)
          .toLowerCase()
  ...
  const handleStatusChange = (record, checked) => {
    ...
    apolloClient
      ...
      .then(res => {
        ...
        if (!data?.status) return InternalErrorMessage();
        message.success(`${record.coupon_name} status updated successfully`);
      });
  };
